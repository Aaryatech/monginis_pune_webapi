...
public interface CrnHsnwiseExcelReportRepository extends JpaRepository<CrnHsnwiseExcelReport, Integer>{
	...
	@Query(value="select UUID() as id, t_credit_note_header.ex_varchar1 as supplier_invoice_no,t_credit_note_details.bill_date as supplier_invoice_date,t_credit_note_header.crn_id as invoice_no,t_credit_note_header.crn_date as invoice_date,t_credit_note_header.fr_id,m_franchisee.fr_name,m_item_sup.item_hsncd,SUM(t_credit_note_details.grn_gvn_qty) as qty,SUM(t_credit_note_details.taxable_amt)as taxable_amt,SUM(t_credit_note_details.cgst_rs)as cgst_rs,SUM(t_credit_note_details.sgst_rs)as sgst_rs,0 as igst_rs,(t_credit_note_details.cgst_per+t_credit_note_details.sgst_per) as tax_rate,t_credit_note_header.crn_grand_total as document_amount,m_franchisee.fr_gst_no,'INDIA' as country,'MAHARASHTRA' as state from t_credit_note_header,t_credit_note_details,m_item_sup,m_franchisee  where m_item_sup.item_id=t_credit_note_details.item_id and t_credit_note_header.crn_id=t_credit_note_details.crn_id and t_credit_note_header.crn_id IN(:crnIdList) And m_franchisee.fr_id=t_credit_note_header.fr_id group by t_credit_note_header.crn_id,item_hsncd",nativeQuery=true)
