...
public interface ItemDiscConfiguredRepository extends JpaRepository<ItemDiscConfigured, Integer>{
	...
	@Query(value="select disc_per from m_fr_discount where is_active=1  and FIND_IN_SET(:id,item_id) and FIND_IN_SET(:frId,franch_id) order by disc_id desc limit 1\n" + 
			"",nativeQuery=true)
