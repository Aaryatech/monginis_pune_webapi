...
public interface PrevItemOrderRepository extends  CrudRepository<Orders, Long>{
 ...
 @Query(value = "SELECT * FROM t_order WHERE t_order.fr_id=:frId AND t_order.item_id IN(:items) AND t_order.order_date=:date AND t_order.menu_id=:menuId", nativeQuery = true)
 List<Orders> findAllOrdersFor1030Order(@Param("items")List<Integer>items ,@Param("frId") String frId,@Param("date") String date,@Param("menuId") String menuId);
