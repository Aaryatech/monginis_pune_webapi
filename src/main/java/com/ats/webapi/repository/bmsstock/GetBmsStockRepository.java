...
public interface GetBmsStockRepository extends JpaRepository<GetBmsStock, Integer>{
	...
	/*
	 * @Query(
	 * value=" SELECT d.bms_stock_deatil_id,u.uom, d.is_del_status, d.rm_type, d.rm_name, d.rm_id, d.rm_uom, d.bms_stock_date, d.bms_stock_id, b.bms_opening_stock, sum(d.store_rec_qty) as store_rec_qty, sum(d.store_rejected_qty) as store_rejected_qty, sum(d.mixing_rec_qty) as mixing_rec_qty,"
	 ...
	 * + "",nativeQuery=true) List<GetBmsStock>
	 * getStockDetails(@Param("fromDate")String fromDate, @Param("toDate")String
	 * toDate, @Param("rmType")int rmType);
	 * 
	 * 
	 * 
	 ...
	 * 
	 * @Query(
	 * value=" SELECT d.bms_stock_deatil_id, d.is_del_status, d.rm_type, d.rm_name, d.rm_id, d.rm_uom, d.bms_stock_date, d.bms_stock_id, f.bms_opening_stock, sum(d.store_rec_qty) as store_rec_qty, sum(d.store_rejected_qty) as store_rejected_qty, sum(d.mixing_rec_qty) as mixing_rec_qty,\r\n"
	 * +
	 * "		sum(d.mixing_receive_rejected_qty) as mixing_receive_rejected_qty, sum(d.mixing_issue_qty) as mixing_issue_qty, sum(d.mixing_return_qty) as mixing_return_qty, sum(d.mixing_rejected) as mixing_rejected,\r\n"
	 * +
	 ...
	 * + "h1.bms_status=:bmsStatus AND d.rm_type=:rmType " + "",nativeQuery=true)
	 * List<GetBmsStock> getStockBetDateMonth(@Param("fromDate")String
	 * fromDate, @Param("toDate")String toDate, @Param("rmType")int
	 * rmType,@Param("bmsStatus") int bmsStatus);
	 * 
	 */
	//Gfpl copy
